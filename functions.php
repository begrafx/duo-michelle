...
update_option('siteurl','http://michellesagara.com');
update_option('home','http://michellesagara.com');
...
/* extra filtering for the_content to remove a leading quotation mark */

function remove_leading_quote($text) {
  $text = ltrim($text, '<p><span class="dquo">“</span>');
  return "<p>" . $text;
}



/* and let's get rid of social's css */
...
/* okay, let's try filtering the sidebar a bit */

function filter_recent_posts( $text ) {
    return ($text . '<li class="rss">Blog<a href="http://msagara.livejournal.com">RSS Feed</a></li>');
}

//add_filter('widget_recent_posts', 'filter_recent_posts', 11); 

// 11 is one above the default priority of 10, meaning it will occur after any other default widget_title filters
...
function boilerplate_widgets_init() {
	...
	// Area 2, located on the homepage
  register_sidebar( array(
		'name' => __( 'Homepage Area', 'boilerplate' ),
		'id' => 'homepage-widget-area',
		'description' => __( 'A widget area to display on the homepage', 'boilerplate' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
...
function boilerplate_posted_in() {
	...
	if ( $tag_list ) {
		$posted_in = __( '<span class="entry-cat">Posted in %1$s and tagged %2$s.</span>', 'boilerplate' );
	} elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$posted_in = __( '<span class="entry-cat">Posted in %1$s. </span>', 'boilerplate' );
	} else {
...
// add book preview size
if ( function_exists( 'add_image_size' ) ) { 
	add_image_size( 'book-preview', 205, 325, false ); 
}

// add series list size
if ( function_exists( 'add_image_size' ) ) { 
	add_image_size( 'series-list', 117, 160, false ); 
}
...
add_filter( 'the_content', 'add_author_spans', 10 );
// add span classes to content 
function add_author_spans($text) {
  $text = str_replace("Michelle Sagara", "<span class=\"sagara\">Michelle Sagara</span>", $text);
  $text = str_replace("Michelle West", "<span class=\"west\">Michelle West</span>", $text);
  $text = str_replace("<span class=\"sagara\">Michelle Sagara</span> West", "Michelle Sagara West", $text);
  return $text;
}
?>
