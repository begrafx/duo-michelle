...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
          ...
          ?>
            ...
            $content = str_replace(']]>', ']]&gt;', $content);
				...
				?>
						...
						<?php // this outputs the other titles in the series
        $series_terms = get_the_terms( 0, 'series' );
        
        
        if ( $series_terms ) 
          
          // remove featured series, if it exists
          unset($series_terms['34']);

          $series_term_slugs = wp_list_pluck( $series_terms, 'slug' );
          $series_title = wp_list_pluck( $series_terms, 'name' );
         // print_r ($series_terms);
          $args = array(
            'post_type' => 'books',
            'order' => 'ASC',
            'posts_per_page' => -1,
            'tax_query' => array(array('taxonomy' => 'series', 'field' => 'slug', 'terms' => $series_term_slugs))
          );
          ?>
        ...
        foreach ($series_terms as $series_term) {
          ...
          $url =  $series_term->slug;
        }
        ?>
        
        <h3>Other books in <a href="/series/<?php echo $url; ?>"><?php echo $title; ?></a></h3>
        <ul>
        <?php while ($my_query->have_posts()) : $my_query->the_post(); ?>
        <?php if ($current_post != the_title('','',false)): ?>
          <li><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></li>
      <?php endif; endwhile; wp_reset_postdata();  ?>	</ul>	
